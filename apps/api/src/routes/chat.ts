...
import { consumeDailyMessage } from '../lib/rateLimit.js';
import { env } from '../env.js';

export const chatRouter = new Hono<{ Variables: ApiVariables }>();
...
chatRouter.post('/', async (c) => {
  ...
  // Soft daily cap — cost guardrail on the Claude copilot.
  const limit = consumeDailyMessage(c.get('userId'), env.copilotDailyMessageCap);
  if (!limit.allowed) {
    return c.json(
      {
        error:
          "You've reached today's chat limit. It resets tomorrow — your schedule keeps running in the meantime.",
        code: 'daily_limit',
      },
      429,
    );
