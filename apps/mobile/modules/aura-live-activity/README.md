# Chronos Live Activity (focus sessions)

A DoorDash-style **Live Activity** for an active task. When a student opens a
task's focus screen (`/tasks/[id]/active`), Chronos shows a glowing countdown —
on the lock screen and Dynamic Island (iOS 16.2+) — that counts down over the
task's `estimatedMinutes`, with pause/resume mirrored live.
...
```
useActiveTask (hooks/useActiveTask.ts)        ← timing state machine, the truth
        │  start / pause / resume / end
        ├──────────────► tasks/[id]/active.tsx (in-app focus screen)
        │                 works everywhere, incl. Expo Go
...
| `ios/AuraLiveActivityModule.swift` | Expo module: `startActivity/updateActivity/endActivity/areActivitiesEnabled` |
...
pnpm --filter @chronos/mobile add -D @bacons/apple-targets

# 2. Generate native projects (creates the AuraTaskActivity target from
#    targets/aura-task-activity/expo-target.config.js)
pnpm --filter @chronos/mobile exec expo prebuild -p ios --clean

# 3. Build & run a dev client on a device
pnpm --filter @chronos/mobile exec expo run:ios --device
#    …or build with EAS:  eas build --profile development -p ios
```

`app.config.js` already sets `ios.infoPlist.NSSupportsLiveActivities: true` and
registers the `@bacons/apple-targets` plugin.

> **Set `ios.appleTeamId` in `app.config.js`.** `@bacons/apple-targets` needs
> your Apple Developer Team ID to sign the widget extension; without it `expo
> prebuild` warns and the iOS build can fail. Set `EXPO_APPLE_TEAM_ID` in the
> environment (the config reads it) or hardcode it under `expo.ios`.
...
- **Android / web / Expo Go / pre-dev-build:** the native module resolves to
  ...
  focus screen still drives the whole experience.
- **Android follow-up:** map sessions to an ongoing notification (there's no
  Live Activity equivalent).

## Known follow-ups

- On app relaunch the native activity's id is lost, so we don't currently
  reconcile/end a pre-existing native activity. Expose `getActiveActivityIds()`
  from the module to fix this.
