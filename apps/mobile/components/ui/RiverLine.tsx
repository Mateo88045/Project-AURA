...
export function RiverLine({ x = 56, style }: RiverLineProps) {
  ...
  return (
    <View style={[styles.fill, style]} pointerEvents="none">
      ...
      <LinearGradient
        ...
        locations={[0, 0.45, 1]}
        style={[styles.strand, { width: GLOW_WIDTH, left: x - GLOW_WIDTH / 2 }]}
      />
      {/* Crisp core — threads exactly through each task dot. */}
      <LinearGradient
        colors={['transparent', colors.accent.sky, colors.accent.blue + '4D']}
        locations={[0, 0.4, 1]}
        style={[styles.strand, { width: CORE_WIDTH, left: x - CORE_WIDTH / 2 }]}
      />
    </View>
  );
}
